...
def _get_combo_config(model: str, dimensions: int) -> dict[str, str] | None:
    ...
    cfg = load_config(
        overrides={"embedding_model": model, "embedding_dimensions": str(dimensions)}
    )
...
def _print_ablation_table(
    ...
    for mode in modes:
        ...
        for i, r in enumerate(results):
            ...
            if i == 0:
                vals = "  ".join(
                    f"{m.get(k, 0.0):.4f}".ljust(val_col) for k in metric_keys
                )
            else:
                bm = baseline["metrics_by_mode"].get(mode, {})
                deltas = []
                for key in metric_keys:
                    delta = m.get(key, 0.0) - bm.get(key, 0.0)
                    sign = "+" if delta >= 0 else ""
                    deltas.append(f"{sign}{delta:.4f}".ljust(val_col))
                vals = "  ".join(deltas)
            print(
                r["model"].ljust(model_col)
                + str(r["dimensions"]).ljust(dims_col)
                + "  "
                + vals
                + tag
            )
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="bookmark-eval-ablation-") as tmpdir:
        ...
        for model, dims in combos:
            ...
            combo_db = (
                Path(tmpdir)
                / f"combo_{model.replace('/', '_').replace('-', '_')}_{dims}.sqlite3"
            )
            ...
            results.append(
                {"model": model, "dimensions": dims, "metrics_by_mode": combo_metrics}
            )
